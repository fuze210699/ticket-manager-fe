<template>
  <div class="h-screen flex flex-col">
    <div class="bg-white border-b border-gray-200 px-4 py-2 flex items-center gap-2">
      ...
      <input v-model="workspaceName" type="text" :placeholder="$t('workspace.namePlaceholder')"
    ...
    <div class="h-screen flex-1 flex flex-col">
      ...
      <VueFlow v-model="elements" @edge-create="handleEdgeCreate" @node-click="openTicketDetails" :pan-on-scroll="true"
        :pan-on-drag="true" :nodes-draggable="true" :nodes-connectable="true" :elements-selectable="true"
        :zoom-on-scroll="true" :zoom-on-pinch="true" :zoom-on-double-click="true" :min-zoom="0.2" :max-zoom="4"
        class="flex-1">
      ...
      <TicketModal v-if="showTicketModal" @close="showTicketModal = false" @save="saveTicket" />

      <MilestoneModal v-if="showMilestoneModal" @close="showMilestoneModal = false" @save="saveMilestone" />

      <DependencyModal v-if="showDependencyModal" @close="showDependencyModal = false" @save="saveDependency" />
    </div>
  </div>
</template>
...
import CustomNode from '@shared/components/ui/CustomNode.vue';
import CustomEdge from '@shared/components/ui/CustomEdge.vue';
